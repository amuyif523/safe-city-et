...
def create_email_verification_token(db: Session, user_id: int) -> str:
    ...
    entry = EmailVerificationToken(user_id=user_id, token=token, expires_at=expires)
    db.add(entry)
    db.commit()
    return token


def consume_email_verification_token(db: Session, token_str: str) -> User:
    entry = db.query(EmailVerificationToken).filter_by(token=token_str).first()
    if not entry or entry.expires_at < datetime.utcnow():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired token.")
    user = db.get(User, entry.user_id)
    if not user:
